...
import type { HoverCardContentProps as HoverCardContentPropsDefault, HoverCardProps } from "@radix-ui/react-hover-card";
import { Root as HoverRoot, Trigger } from "@radix-ui/react-hover-card";
...
const Root = (props: HoverCardProps) => {
  ...
  return (
    <CustomContext.Provider value={React.useMemo(() => ({ onOpenChange: setOpen, open }), [setOpen, open])}>
      <HoverRoot {...props} open={open} onOpenChange={setOpen} />
 ...
 * Content
 * -------------------------------------------------------------------------------------------------
 * This is mainly a wrapper around the 'hovercard-content' Content primitive with some custom styling and animation.
...
type HoverCardContentElement = React.ElementRef<typeof HoverCard.Content>;

interface HoverCardContentProps extends HoverCardContentPropsDefault {
  isDisabled?: boolean;
}

const Content = React.forwardRef<HoverCardContentElement, HoverCardContentProps>(function PopoverContent(
    ...
    <AnimatePresence>
      {open ? (
        ...
        <HoverCard.Portal forceMount>
          <HoverCard.Content {...rest} ref={forwardedRef} className={clsx(defaultContentStyle, className)} asChild>
            <motion.div
              initial={"closed"}
              animate={"open"}
              exit={"closed"}
...
const variants: Record<NoUndefined<HoverCardContentProps["side"]>, Variants> = {
  bottom: {
