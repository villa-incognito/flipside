...
const SQLTypes = "";
const SQLKeywords = "and any as asc between by case desc distinct exists from group by having in inner is join join left like limit not null or order outer right rownum select select top union where";
const defaults = {
...
function completeFromSchema(dbDef) {
    const top = new CompletionLevel();
    let defaultDatabase = top.setChild("");
    dbDef.forEach((db) => {
        const dbCompletion = top.setChild(db.name);
        // add custom table types to create classes for styling
        top.list = dbDef.map((db) => ({ label: db.name, type: `autocomplete-${kebabCase(db.name)}` }));
        dbCompletion.list = db.schemas.map((schema) => ({
            label: schema.name,
            type: `autocomplete-${kebabCase(db.name)}-${kebabCase(schema.name)}`,
        }));
        db.schemas.forEach((schema) => {
            const schemaCompletion = dbCompletion.setChild(schema.name);
            schemaCompletion.list = schemaCompletion.list.concat(schema.tables.map((table) => ({ label: table.name, type: "autocomplete-table" })));
            schema.tables.forEach((table) => {
                const tableCompletion = schemaCompletion.setChild(table.name);
                tableCompletion.list = table.columns
                    .filter((val) => !val.startsWith("_"))
                    .map((val) => ({ label: val, type: "autocomplete-column" }));
            });
        });
    });
    return (context) => {
        ...
        for (let name of parents) {
            while (!level.children || !level.children[name]) {
                if (level == top)
                    level = defaultDatabase;
                else
                    return null;
            }
            level = level.setChild(name);
        }
...
function schemaCompletionSource(config) {
    return config.schema ? completeFromSchema(config.schema) : () => null;
}
/**
FIXME remove on 1.0 @internal
*/
